...
bedClrEigenBin<-function(pca,namePCA){
  ...
  export.bed(pca2bed,paste0(otherDataDir,"/bin3q_",namePCA,"_rgb.bed"))
  pca2bed<-read.delim(paste0(otherDataDir,"/bin3q_",namePCA,"_rgb.bed"),header=F)
  pca2bed<-cbind(pca2bed,md[!naidx,c("itemRGB")])
  write.table(pca2bed,
              file=paste0(otherDataDir,"/bin3q_",namePCA,"_rgb.bed"),
              sep="\t",row.names=F,col.names=F,quote=F)
  # add trackline (but not necessary)
  fConn <- file(paste0(otherDataDir,"/bin3q_",namePCA,"_rgb.bed"), 'r+')
...
bedClrSwitch<-function(pca,namePCA){
  ...
  export.bed(pca2bed,paste0(otherDataDir,"/switchBin_",namePCA,"_rgb.bed"))
  pca2bed<-read.delim(paste0(otherDataDir,"/switchBin_",namePCA,"_rgb.bed"),header=F)
  pca2bed<-cbind(pca2bed,md[!naidx,c("itemRGB")])
  write.table(pca2bed,
              file=paste0(otherDataDir,"/switchBin_",namePCA,"_rgb.bed"),
              sep="\t",row.names=F,col.names=F,quote=F)
  # add trackline (but not necessary)
  fConn <- file(paste0(otherDataDir,"/switchBin_",namePCA,"_rgb.bed"), 'r+')
  Lines <- readLines(fConn)
  writeLines(c(trackLine, Lines), con = fConn)
  close(fConn)
}
...
bins<-readRDS(paste0(otherDataDir,"/",pcas$E1[pcas$SMC==refset]))
mcols(bins)<-NULL
...
for (g in 1:length(otherPCAs)){
  ...
  pca1<-readRDS(paste0(otherDataDir,"/",pcas$E1[pcas$SMC==grp]))
  pca2<-readRDS(paste0(otherDataDir,"/",pcas$E2[pcas$SMC==grp]))
...
write.table(statdfwide,file=paste0(projectDir,"/switchStats.tsv"),sep="\t",
            row.names=F)
...
rnaseqLFCpath<-paste0(projectDir,"/tracks/2021_RNAseq_MDas/p0.05_lfc0.5_filtCycChrAX")
...
bins<-readRDS(paste0(otherDataDir,"/",pcas$E2[pcas$SMC==refset]))
mcols(bins)<-NULL
...
refset=refPCAs[1]
featureTypePath<-paste0(projectDir,"/tracks")
bins<-readRDS(paste0(otherDataDir,"/",pcas$E2[pcas$SMC==refset]))
mcols(bins)<-NULL
bins<-getChromMarks(bins,rnaTPM)
...
pdf(file=paste0(finalFigDir,"/Fig_S4_compartmentSwitch.pdf"),width=10,height=10,
    paper="a4")
...
explainTerciles<-function(binRDS){
  ...
  p1<-ggplot(data.frame(binRDS),aes(x=1:length(binRDS),y=score)) + geom_line() +
...
p
ggsave(paste0(finalFigDir,"/Fig_S4_explainTerciles.pdf"),p,device="pdf",width=16,height=10,units="cm")
